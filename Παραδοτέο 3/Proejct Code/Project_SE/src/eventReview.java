public class eventReview extends event{
    //attributes
    public int eventRevId;
    public string eventRevTitle;
    public string eventRevText;
    public int eventRevScore;

    //constructor

    public eventReview() {
    }

    //methods


    //getters and setters

    public int getEventRevId() {
        return eventRevId;
    }

    public void setEventRevId(int eventRevId) {
        this.eventRevId = eventRevId;
    }


    public string getEventRevTitle() {
        return eventRevTitle;
    }

    public void setEventRevTitle(string eventRevTitle) {
        this.eventRevTitle = eventRevTitle;
    }


    public string getEventRevText() {
        return eventRevText;
    }

    public void setEventRevText(string eventRevText) {
        this.eventRevText = eventRevText;
    }


    public int getEventRevScore() {
        return eventRevScore;
    }

    public void setEventRevScore(int eventRevScore) {
        this.eventRevScore = eventRevScore;
    }
}
